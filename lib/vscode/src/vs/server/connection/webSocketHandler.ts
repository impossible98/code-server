 ...
 */
export class WebSocketHandler extends Disposable {
	...
	constructor(private readonly netServer: net.Server, private readonly environmentService: IEnvironmentServerService, private readonly logService: ILogService) {
		...
		this.netServer.on('upgrade', this.upgradeListener);
	}

	upgradeListener = (req: IncomingMessage, socket: net.Socket, head: Buffer) => {
		...
		try {
			...
			connectionOptions = parseQueryConnectionOptions(searchParams);
		} catch (error: unknown) {
	...
	override dispose(): void {
		...
		if (this.netServer) {
			this.netServer.off('upgrade', this.upgradeListener);
		}
	}
}
