...
import { WebSocketHandler } from './connection/webSocketHandler';
import { EnvironmentServerService, IEnvironmentServerService } from './environmentService';
import { NetRequestHandler } from './net/netRequestHandler';
...
export class ServerProcessMain extends Disposable implements IServerProcessMain {
	...
	public async startup({ listenWhenReady = true }: IServerProcessMainStartupOptions = {}): Promise<NetServer> {
		...
		const netRequestHandler = new NetRequestHandler(netServer, environmentServerService, logService);
		...
		this._register(combinedDisposable(instantiationService.createInstance(LogsDataCleaner), instantiationService.createInstance(ErrorTelemetry), netRequestHandler, websocketHandler, ipcServer));
